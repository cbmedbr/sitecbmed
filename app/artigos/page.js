    ...
    <>
      ...
      <section className="py-12 bg-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-xs font-bold uppercase tracking-widest text-brand-500 mb-6">Artigo em destaque</p>
            ...
            <div className="group grid grid-cols-1 lg:grid-cols-2 gap-0 bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow">
              ...
              <div className="p-8 md:p-10 flex flex-col justify-center">
                <div className="flex items-center gap-3 text-xs text-ink-muted mb-4">
                  <span className="font-mono">{formatarData(recentes.data)}</span>
                  <span>·</span>
                  <span className="font-mono">{recentes.tempoLeitura} de leitura</span>
                </div>
                <h2 className="font-serif font-semibold text-ink text-2xl md:text-3xl mb-3 group-hover:text-brand-600 transition-colors leading-tight">
                  {recentes.titulo}
                </h2>
                <p className="text-ink-light leading-relaxed mb-6">{recentes.resumo}</p>
      ...
      {/* ── Demais artigos ── */}
      <section className="py-8 pb-20 bg-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-xs font-bold uppercase tracking-widest text-ink-muted mb-8">Todos os artigos</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="group card h-full flex flex-col overflow-hidden">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex items-center gap-2 text-xs text-ink-muted mb-3">
                      <span className="font-mono">{formatarData(artigo.data)}</span>
                      <span>·</span>
                      <span className="font-mono">{artigo.tempoLeitura}</span>
                    </div>
                    <h3 className="font-serif font-semibold text-ink mb-2 leading-snug group-hover:text-brand-600 transition-colors flex-1">
                      {artigo.titulo}
                    </h3>
            ...
            <div className="card flex flex-col items-center justify-center p-8 text-center border-dashed border-2 border-ink/10 bg-white/50 min-h-[300px]">
              ...
              <h3 className="font-serif font-semibold text-ink mb-2">Novo artigo em breve</h3>
              <p className="text-ink-muted text-sm">Publicamos novos conteúdos científicos toda semana. Volte em breve.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 bg-forest">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-serif font-semibold text-white text-2xl mb-3">Tem dúvidas sobre Cannabis Medicinal?</h2>
          <p className="text-white/70 mb-6">Nossa equipe especializada está disponível para orientar pacientes e médicos prescritores.</p>
          <MagneticButton href="/acolhimento" className="btn-primary">
            Iniciar Acolhimento
          </MagneticButton>
        </div>
      </section>
